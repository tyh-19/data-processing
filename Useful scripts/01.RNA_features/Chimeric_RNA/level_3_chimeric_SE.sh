...
STAR --genomeDir /data/taoyuhuan/projects/exOmics_RNA/bin/genome/chimera-star-index \
          --readFilesIn ${input}/${sample}/circRNA.fa.gz \
          --runThreadN 16 \
...
/data/taoyuhuan/projects/exOmics_RNA/bin/scripts/count_reads.py count_circrna -s ${strandness} -i ${output}/${sample}/${sample}Aligned.out.bam -o ${output}/counts/${sample}/${sample}
