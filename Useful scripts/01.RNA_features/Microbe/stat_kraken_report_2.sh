...
echo "sample	human_percent	fungi_percent	bacteria_percent	archea_percent	virus_percent	human_readnum	fungi_readnum	bacteria_readnum	archea_readnum	virus_reaednum	genus_num(read pairs > 2) specie_num" > ${output}/stat_kraken_report.txt
...
genus=`cat ${input}/report/${sample} | awk '($4=="G"&&$2>2) {print $0}' | wc -l`
...
sed -i '1i\sample	human_percent	human_read	microbe_percent	microbe_read	genus_num(read pairs > 2)	specie_num' ${output}/stat_kraken_report_summary.tmp
