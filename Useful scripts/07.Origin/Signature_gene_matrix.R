...
parser <- ArgumentParser(description='Signature Matrix v1')
...
parser$add_argument('-e', '--TPM_cutoff', type='double', required=TRUE,
                    help='TPM cutoff, maximum expressionn larger than this cutoff. Default: 20.')
parser$add_argument('-ts','--TSS_cutoff', type='double', required=TRUE,
                    help='TSS cutoff, minimum tissue specific score larger than this cutoff. Default: 0.95.')
...
#function
mean_TSS <- function(mat,sample_info,TPM_cutoff=20,TSS_cutoff=0.95,outdir){
  ...
  write.table(signature_matrix,paste0(outdir,"/","signature_genes_",TPM_cutoff,"_",TSS_cutoff,".txt"),quote = FALSE,sep = "\t",row.names = TRUE,col.names = TRUE)
  ...
  write.table(row_annotation,paste0(outdir,"/","gene_list_",TPM_cutoff,"_",TSS_cutoff,".txt"),quote=FALSE,sep="\t",row.names = TRUE, col.names = TRUE)
  return(TSG_plot)
}
...
TSG_plot <- mean_TSS(mat,sample_info,TPM_cutoff,TSS_cutoff,outdir)
...
save_pheatmap_pdf(plot_sample, paste0(outdir,"/","signature_genes_samples_",TPM_cutoff,"_",TSS_cutoff,".pdf"))
dev.off()
...
save_pheatmap_pdf(plot_tissue, paste0(outdir,"/","signature_genes_tissues_",TPM_cutoff,"_",TSS_cutoff,".pdf"))
dev.off()
